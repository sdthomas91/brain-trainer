...
2. Music Playing - there is a slight issue with the autoplay function in that it doesn't always play on page refresh. Fresh page load, or page load from clicking the link is usually successful. Look over a number of articles and tutorials but it would seem the setup is correct. Will need to be rectified in a later version. In addition the music toggle doesnt start the music if it does bit autoplay initially. This is problematic on some devices/browsers that block autoplay. For now the toggle works when music is playing and the music plays more often than it does not. Does not interfere with gameplay so not a major bug.
